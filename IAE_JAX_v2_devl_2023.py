...
class IAE(object):
    ...
    def display(self,epoch,epoch_time,train_acc,rel_acc,pref="Learning stage - ",niter=None):
        ...
        bar = bar + ' |'
        bar = bar + str(int(100 * percent_time))+'%'
        m, s = divmod(int(epoch*epoch_time), 60)
        h, m = divmod(m, 60)
        time_run = ' [{:d}:{:02d}:{:02d}<'.format(h, m, s)
        m, s = divmod(int((niter-epoch)*epoch_time), 60)
        h, m = divmod(m, 60)
        time_run += '{:d}:{:02d}:{:02d}]'.format(h, m, s)

        sys.stdout.write('\033[2K\033[1G')
        if epoch_time > 1:
            print(pref+'epoch {0}'.format(epoch)+'/' +str(niter)+ ' -- loss  = {0:e}'.format(float(train_acc)) + ' -- loss rel. var. = {0:e}'.format(float(rel_acc))+bar+time_run+'-{0:0.4} '.format(epoch_time)+' s/epoch', end="\r")
        if epoch_time < 1:
            print(pref+'epoch {0}'.format(epoch)+'/' +str(niter)+ ' -- loss  = {0:e}'.format(float(train_acc)) + ' -- loss rel. var. = {0:e}'.format(float(rel_acc))+bar+time_run+'-{0:0.4}'.format(1./epoch_time)+' epoch/s', end="\r")
        #sys.stdout.flush()
    ...
    def init_parameters(self):
        ...
        if self.sparse_code:
            ...
            for i in range(self.niter_sparse):
                ...
                self.Params["step_size_"+str(i)] =  np.zeros((1 ,))
        ...
        if self.Model is not None:
            ...
            if self.sparse_code:
                ...
                for i in range(self.niter_sparse):
                    self.Params["step_size_"+str(i)] =  self.Model["Params"]["step_size_"+str(i)]
    ...
    def update_parameters(self, Params):
        ...
        if self.sparse_code:
            ...
            for i in range(self.niter_sparse):
                self.Params["step_size_"+str(i)] = Params["step_size_"+str(i)]
    ...
    def learnt_params_init(self):
        ...
        if self.sparse_code:
            ...
            for i in range(self.niter_sparse):
                Params["step_size_"+str(i)] = self.Params["step_size_"+str(i)]
        ...
        if self.sparse_code:
            ...
            if W is None:
                for r in range(self.niter_sparse):
                    Z = Z - np.exp(self.Params["step_size_"+str(r)])*np.dot(np.dot(Z,PhiE)-PhiX,np.transpose(PhiE))/np.linalg.norm(PhiE2) # Pas forcément top, au minimum faire un FISTA
                Z1 = Z-np.exp(self.Params["thd"])
                Z2  = -Z-np.exp(self.Params["thd"])
                Lambda = Z1*(Z1 > 0) - Z2*(Z2>0)
            else:
                for r in range(self.niter_sparse):
                    Z = Z - np.exp(W["step_size_"+str(r)])*np.dot(np.dot(Z,PhiE)-PhiX,np.transpose(PhiE))/np.linalg.norm(PhiE2) # Pas forcément top, au minimum faire un FISTA
                Z1 = Z-np.exp(W["thd"])
                Z2  = -Z-np.exp(W["thd"])
                Lambda = Z1*(Z1 > 0) - Z2*(Z2>0)
            if self.simplex:
...
#####################################
    ...
    def learn_unrolled_bsp(self, X, NLayers=5,Amplitude=None, Lambda0=None, Amplitude0=None, niter=None, optim=None, step_size=None):
        ...
        for r in range(NLayers):
            ...
            params_unroll["W"+str(r)] = W0/onp.linalg.norm(W0) # Hessian-like
            params_unroll["alphaA"+str(r)] = step_size # step size Amplitude
        ...
        def unroll_model(X,Params_bsp,Params,Pgrad=None):
            ...
            for l in range(NLayers):
                ...
                # Update
                amp -= Params["alphaA"+str(l)]*g_amp
                lamb -= np.dot(g_lamb,Params["W"+str(l)])
            return amp,lamb
        ...
        def get_unroll_values(X,Params_bsp,Params):
            ...
            for l in range(NLayers):
                ...
                g_lamb,g_amp = get_grad(lamb,amp)
                # Update
                amp -= 1./Params["alphaA"+str(l)]*g_amp
                lamb -= 1./Params["alphaL"+str(l)]*np.dot(g_lamb,Params["W"+str(l)])
                Q = {}
                Q["Amplitude"] = amp
                Q["Lambda"] = lamb
                Q["grad"] = g_lamb,g_amp
    ...
    def barycentric_span_projection_fast(self, X, params_unroll=None,NLayers=5, Amplitude=None, Lambda0=None, Amplitude0=None):
        ...
        def unroll_model(X,Params_bsp,Params):
            ...
            for l in range(NLayers):
                ...
                # Update
                amp -= Params["alphaA"+str(l)]*g_amp
                lamb -= Params["alphaL"+str(l)]*np.dot(g_lamb,Params["W"+str(l)])
            return amp,lamb
